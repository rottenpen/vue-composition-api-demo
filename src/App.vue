<template>
  <div id="app">
    <section class="todoapp">
      ...
      <section class="main" v-show="state.todoList.length">
        ...
        <ul class="todo-list">
          <li v-for="todo in filteredTodos"
              class="todo"
              :key="todo.id"
              :class="{ completed: todo.completed, editing: todo === state.editTodo }">
            <div class="view">
            ...
            <input class="edit" type="text"
                    v-model="todo.title"
                    v-todo-focus="todo === state.editTodo"
                    @blur="doneEdit(todo)"
      ...
      <footer class="footer" v-show="state.todoList.length">
        ...
        <button class="clear-completed" @click="removeCompleted" v-show="state.todoList.length > remaining">
          Clear completed
        </button>
      </footer>
    </section>
...
const STORAGE_KEY = 'todo-vuejs-3.x'
const todoStorage = {
  uid: 0,
  fetch () {
    const todoList: Todo[] = JSON.parse(localStorage.getItem(STORAGE_KEY) || '[]')
...
const getHash = (): Visibility => {
  const hash = window.location.hash.replace(/#\/?/, '')
  if (hash === 'all' || hash === 'completed' || hash === 'active') {
    return hash
  }
  return 'all'
}
...
export default defineComponent({
  setup() {
    ...
    // 计算属性 => computed
    const remaining = computed(() => filters.active(state.todoList).length)
    const filteredTodos = computed(() => filters[state.visibility](state.todoList))
    ...
    function removeCompleted () {
      state.todoList = filters.active(state.todoList)
    }
